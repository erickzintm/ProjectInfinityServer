...
<#
    ...
    Start script generated by ServerPackCreator 8.1.0.
    The template which was used in the generation of this script can be found at:
    https://github.com/Griefed/ServerPackCreator/blob/8.1.0/serverpackcreator-api/src/main/resources/de/griefed/resources/server_files/default_template.ps1
...
Function global:CleanServerFiles
{
    ...
    # Protected directories - these will NEVER be deleted to preserve user modifications
    $ProtectedDirs = @(
        "config"
        "mods"
        "world"
    ...
    ForEach ($FileToRemove in $FilesToRemove) {
        # Check if this item is a protected directory
        $IsProtected = $false
        ForEach ($ProtectedDir in $ProtectedDirs) {
            if ($FileToRemove -eq $ProtectedDir) {
                $IsProtected = $true
                Write-Host "PROTECTED: Skipping deletion of ${FileToRemove} (user modifications preserved)" -ForegroundColor Yellow
                break
            }
        }
